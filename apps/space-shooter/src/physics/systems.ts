...
import { addResource, getResourceValue, queryEntities, readEvents, removed } from "iris-ecs";
...
export function updateMovement(world: World): void {
  ...
  queryEntities(world, [Transform, Movement], (entity) => {
...
export function updateSpatialHashing(world: World): void {
  const map = getResourceValue(world, SpatialHash, "map")!;

  const { getPosition } = transformActions(world);

  queryEntities(world, [Transform], (entity) => {
    const [x, y] = getPosition(entity);
...
export function cleanupSpatialHashMap(world: World): void {
  const map = getResourceValue(world, SpatialHash, "map")!;
