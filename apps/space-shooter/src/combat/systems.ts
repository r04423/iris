import type { Entity, EntityId, World } from "iris-ecs";
import {
  addResource,
  emitEvent,
  getResourceValue,
...
export function handleShooting(world: World): void {
  ...
  const { canShoot: canShootAction, getShootCooldownState, setShootCooldownState } = combatActions(world);
  const { getInputFire } = inputActions(world);
  const { spawnBullet } = combatActions(world);

  const player = queryFirstEntity(world, [IsPlayer, Transform, ShootCooldown]);
...
export function updateBullets(world: World): void {
  ...
  queryEntities(world, [IsBullet, Bullet, Transform], (entity) => {
...
export function updateBulletCollisions(world: World): void {
  ...
  queryEntities(world, [IsBullet, Bullet, Transform], (bullet) => {
    const [bx, by] = getPosition(bullet);

    map.getNearbyEntities(bx, by, hitRadius, nearby);

    let hitEnemy: EntityId | undefined;
    for (let i = 0; i < nearby.length; i++) {
      const entity = nearby[i]!;
      if (!isEntityAlive(world, entity)) continue;
      if (!hasComponent(world, entity, IsEnemy)) continue;
...
export function pushEnemies(world: World): void {
  const map = getResourceValue(world, SpatialHash, "map")!;
  const nearby = getResourceValue(world, ScratchEntities, "entities")!;
  ...
  queryEntities(world, [IsPlayer, Transform, Movement], (player) => {
    ...
    for (let i = 0; i < nearby.length; i++) {
      ...
      if (!hasComponent(world, entity, IsEnemy)) {
        continue;
      }
...
export function handlePlayerHit(world: World): void {
...
export function tickShieldVisibility(world: World): void {
  const delta = getResourceValue(world, Time, "delta") ?? 0;
  ...
  queryEntities(world, [ShieldVisibility], (entity) => {
