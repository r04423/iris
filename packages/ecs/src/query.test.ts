...
describe("Query", () => {
  ...
  describe("fetchFirstEntity", () => {
    ...
    it("works with change detection modifiers", () => {
      const world = createWorld();
      const Health = createEntity(world);
      ...
      const first1 = fetchFirstEntity(world, added(Health));
      assert.strictEqual(first1, entity);

      // Second call should return undefined (lastTick updated)
      const first2 = fetchFirstEntity(world, added(Health));
  ...
  describe("Change Detection - added()", () => {
    it("matches entities with component added since last query execution", () => {
      const world = createWorld();
      const Health = createEntity(world);
      ...
      const first = [...fetchEntities(world, added(Health))];
      assert.strictEqual(first.length, 1);
      assert.strictEqual(first[0], entity1);

      // Second query at same tick: lastTick updated to 1, addedTick(1) <= lastTick(1), no match
      const second = [...fetchEntities(world, added(Health))];
      assert.strictEqual(second.length, 0);
    });

    it("matches newly added entities after tick advances", () => {
      const world = createWorld();
      const Health = createEntity(world);
      ...
      [...fetchEntities(world, added(Health))]; // consume entity1, updates lastTick to 1

      // Advance tick and add new entity
      buildSchedule(world);
    ...
    it("implicitly includes component in filter (entity must have component)", () => {
      const world = createWorld();
      const Health = createEntity(world);
  ...
  describe("Change Detection - changed()", () => {
    it("matches entities with component changed since last query execution", () => {
      const world = createWorld();
      ...
      [...fetchEntities(world, changed(Position))]; // consume the initial add

      // Advance tick and modify
      buildSchedule(world);
      executeSchedule(world);
    ...
    it("includes additions (add counts as change)", () => {
      const world = createWorld();
      const Health = createEntity(world);
      ...
      const results = [...fetchEntities(world, changed(Health))];
      assert.strictEqual(results.length, 1);
    });

    it("does not match without modification after lastTick update", () => {
      const world = createWorld();
      ...
      [...fetchEntities(world, changed(Position))]; // updates lastTick

      // No change since last query
      const results = [...fetchEntities(world, changed(Position))];
      assert.strictEqual(results.length, 0);
    });
  });
  ...
  describe("Change Detection - Combined Modifiers", () => {
    it("combines added() with regular component requirements", () => {
      const world = createWorld();
      const Position = createEntity(world);
      const Velocity = createEntity(world);
      ...
      const results = [...fetchEntities(world, added(Position), Velocity)];
      assert.strictEqual(results.length, 1);
      assert.strictEqual(results[0], entity1);
    });

    it("combines changed() with not() exclusions", () => {
      const world = createWorld();
      ...
      [...fetchEntities(world, changed(Position))]; // consume initial adds
      ...
      // entity2 is Dead, should be excluded
      const results = [...fetchEntities(world, changed(Position), not(Dead))];
      assert.strictEqual(results.length, 1);
      assert.strictEqual(results[0], entity1);
    });

    it("multiple change modifiers require ALL to match (AND semantics)", () => {
      const world = createWorld();
      const Health = createEntity(world);
      ...
      const results = [...fetchEntities(world, added(Health), added(Mana))];
      assert.strictEqual(results.length, 1);
      assert.strictEqual(results[0], entity1);
    });

    it("combines added() with pair relations", () => {
      const world = createWorld();
      ...
      const results = [...fetchEntities(world, added(pair(ChildOf, parent)), Active)];
      assert.strictEqual(results.length, 1);
      assert.strictEqual(results[0], child1);
    });

    it("combines added() with changed() in single query", () => {
      const world = createWorld();
      ...
      [...fetchEntities(world, added(NewState), changed(Position))]; // consume

      buildSchedule(world);
      executeSchedule(world);

      // Both happen: modify Position, add NewState
      setComponentValue(world, entity, Position, "x", 100);
      addComponent(world, entity, NewState);

      const results = [...fetchEntities(world, added(NewState), changed(Position))];
      assert.strictEqual(results.length, 1);
      assert.strictEqual(results[0], entity);
    });
  });
  ...
  describe("Change Detection - Query lastTick Isolation", () => {
    it("different queries maintain independent lastTick", () => {
      const world = createWorld();
      const Health = createEntity(world);
      const Mana = createEntity(world);
      ...
      const healthResults1 = [...fetchEntities(world, added(Health))];
      assert.strictEqual(healthResults1.length, 1);

      // Query 2: added(Mana) - independent, should still see the entity
      const manaResults = [...fetchEntities(world, added(Mana))];
      assert.strictEqual(manaResults.length, 1);

      // Query 1 again: should be empty (its own lastTick was updated)
      const healthResults2 = [...fetchEntities(world, added(Health))];
      assert.strictEqual(healthResults2.length, 0);
    });

    it("updates lastTick even when no entities match", () => {
      const world = createWorld();
      const Health = createEntity(world);

      [...fetchEntities(world, added(Health))]; // no entities, updates lastTick to 1

      const entity = createEntity(world);
      addComponent(world, entity, Health); // added at tick 1

      // Same tick: entity was added at tick 1, but lastTick is also 1
      // addedTick (1) <= lastTick (1), so no match
      const results = [...fetchEntities(world, added(Health))];
      assert.strictEqual(results.length, 0);
    });
  });
  ...
  describe("Change Detection - Per-System Isolation", () => {
    it("same query in different systems maintains independent lastTick", () => {
      const world = createWorld();
    ...
    it("systems do not see changes from prior ticks after lastTick updates", () => {
      const world = createWorld();
      ...
      buildSchedule(world);
      executeSchedule(world); // tick 2: sees 1 (entity added at tick 1)
      executeSchedule(world); // tick 3: sees 0 (lastTick updated to 2)
      executeSchedule(world); // tick 4: sees 0

      assert.deepStrictEqual(results, [1, 0, 0]);
    });

    it("system sees changes made by earlier system in same tick", () => {
      const world = createWorld();
    ...
    it("outside-scheduler query uses self lastTick, not system lastTick", () => {
      const world = createWorld();
      ...
      // Query outside any system - uses lastTick.self
      const outsideResults1 = [...fetchEntities(world, added(Health))];
      assert.strictEqual(outsideResults1.length, 1);

      // Same query outside again - lastTick.self was updated
      const outsideResults2 = [...fetchEntities(world, added(Health))];
      assert.strictEqual(outsideResults2.length, 0);

      // Now use same query inside a system
      let systemResults = 0;
      addSystem(world, function systemA() {
        for (const _ of fetchEntities(world, added(Health))) {
          systemResults++;
      ...
      // System's lastTick.bySystemId["systemA"] was 0, so it should see the entity
      // even though outside query already consumed it
      assert.strictEqual(systemResults, 1);
    });

    it("query metadata stores per-system ticks correctly", () => {
      const world = createWorld();
      const Health = defineComponent("HealthMeta", { value: Type.f32() });

      const entity = createEntity(world);
      addComponent(world, entity, Health, { value: 100 });

      const query = ensureQuery(world, added(Health));

      // Outside: self tick
      assert.strictEqual(query.lastTick.self, 0);
      [...fetchEntitiesWithQuery(world, query)];
    ...
    it("changed() modifier respects per-system isolation", () => {
      const world = createWorld();
  ...
  describe("Change Detection - Edge Cases", () => {
    it("updates lastTick even when loop exits early via break", () => {
      const world = createWorld();
      const Health = createEntity(world);
    ...
    it("re-added component matches added()", () => {
      const world = createWorld();
      ...
      const addedResults = [...fetchEntities(world, added(Shield))];
      assert.strictEqual(addedResults.length, 1);
      assert.strictEqual(addedResults[0], entity);
    });

    it("entity destruction removes entity from change detection queries", () => {
      const world = createWorld();
      const Health = createEntity(world);
      ...
      // Entity appears in added() query before destruction
      const beforeResults = [...fetchEntities(world, added(Health))];
      assert.ok(beforeResults.includes(entity));

      destroyEntity(world, entity);

      // Entity no longer appears after destruction (row removed from archetype)
      const afterResults = [...fetchEntities(world, added(Health))];
      assert.strictEqual(afterResults.includes(entity), false);
    });

    it("destroyed entity does not appear in change detection queries", () => {
      const world = createWorld();
      const Health = createEntity(world);
